...
if(isset($_SESSION["bool"])  and $_SESSION["bool"] == true ){
    $_SESSION["sessione"] = $_SESSION["preSessione"];
    $_SESSION["tutorial"] = $_SESSION["preTutorial"];
...
<body class="bg-info containter">
    ...
    <div id="title" class="bg-primary border-bottom border-dark">
        <h1 class="display-4 text-center ">Associa speaker alla presentazione di un tutorial</h1>
    </div>
    <div>
        ...
        <div id='form' class="col">
            <h2 id="history">
                <?php
                    ...
                    } else if(isset($_POST["checkedTutorial"])){
                        echo "Sessione: " . $_SESSION["sessione"] . " Tutorial: " . strtok($_POST["checkedTutorial"][0], "-");
                    } else if($GLOBALS["bool"] == true){
                        echo "Sessione: " . $_SESSION["sessione"] . " Tutorial: " . $_SESSION["tutorial"];
                    }
                ?>
            </h2>
                ...
                <div class="form-group m-auto w-75">
                    ...
                    <div class="tab">
                        ...
                        <table class="table table-hover bg-light h-100">
                            ...
                            <tbody id="tbSpeaker">
                                <?php
                                    if(isset($_POST["checkedTutorial"])){
                                        stampaSpeaker();
                                    } else if($GLOBALS["bool"]){
                                        stampaSpeaker();
                                    }
                                ?>
                            </tbody>
    ...
    function stampaTutorial()
    {
        ...
        if ($sql->rowCount() > 0) {
            while ($row = $sql->fetch(PDO::FETCH_ASSOC)) {
                ...
                //select degli speaker
                $conn2 = ConnSQL::DBConnection();

                $tmp_sessione = $row["CodiceSessionePresentazione"];
                $tmp_tutorial = $row["CodicePresentazione"];

                $listaSpeakers = "";

                $sql2 = $conn2->prepare("SELECT UsernameUtente
                                           FROM SPEAKER, PRESENTAZIONE_TUTORIAL
                                          WHERE CodiceSessioneTutorial = '$tmp_sessione'
                                            and CodiceTutorial = '$tmp_tutorial'
                                            and UsernameSpeaker = UsernameUtente");
                $sql2->execute();

                if ($sql2->rowCount() > 0) {
                    $row2 = $sql2->fetch(PDO::FETCH_ASSOC);
                    $listaSpeakers = $row2["UsernameUtente"];
                ...
                $out .= '<tr> <td><input type="radio" name="checkedTutorial[]" value="' . $row['CodicePresentazione'] . "-" . $row['CodiceSessionePresentazione'] . '"</td>';
                $out .= "<td>" . $row['Titolo'] . "</td>";
                $out .= "<td>" . $row['CodicePresentazione'] . "</td>";
                $out .= "<td>" . $row['CodiceSessionePresentazione'] . "</td>";
                $out .= "<td>" . $row['Abstract'] . "</td>";
                $out .= "<td>" . $listaSpeakers . "</td></tr>";
            }
            echo $out;
        }
        $conn = null;
    ...
    function stampaSpeaker()
    {
        $conn = ConnSQL::DBConnection();

        if(isset($_POST["checkedTutorial"])){
            $_SESSION["tutorial"] = strtok($_POST["checkedTutorial"][0], "-");
        }

        $sessione = $_SESSION["sessione"];
        $tutorial = $_SESSION["tutorial"];

        $sql = $conn->prepare("SELECT UsernameUtente, Foto, NomeUni, NomeDip
                                 FROM SPEAKER
                                WHERE UsernameUtente not in(SELECT UsernameSpeaker
                                                              FROM PRESENTAZIONE_TUTORIAL
                                                             WHERE CodiceTutorial = '$tutorial'
                                                               and CodiceSessioneTutorial = '$sessione'
                                                               and UsernameUtente = UsernameSpeaker)");
        $sql->execute();
        ...
        if ($sql->rowCount() > 0) {
            while ($row = $sql->fetch(PDO::FETCH_ASSOC)) {
                $out .= "<tr> <td> <input type='checkbox' name='checkedSpeaker[]' value='" . $row['UsernameUtente'] . "'</td>";
                $out .= "<td>" . $row['UsernameUtente'] . "</td>";
                $out .= "<td>" . $row['NomeUni'] . "</td>";
                $out .= "<td>" . $row['NomeDip'] . "</td></tr>";
            }
            echo $out;
        }
        $conn = null;
    ...
    function associaSpeaker()
    {
        ...
        $sessione = $_SESSION["sessione"];
        $tutorial = $_SESSION["tutorial"];
        $speakers = $_POST["checkedSpeaker"];

        $sql = $conn->prepare("CALL AssociaSpeaker(:UsernameSpeaker, :CodiceTutorial, :CodiceSessioneTutorial)");
        
        for($i = 0; $i < count($speakers); $i++){
            $sql->execute([
                'UsernameSpeaker' => $speakers[$i],
                'CodiceTutorial' => $tutorial,
                'CodiceSessioneTutorial' => $sessione
            ]);
        }
        $conn = null;
    }
...
var currentTab = <?php 
                    if(isset($_POST["checkedSessione"])){
                        ...
                        echo 2;
                    } else if(isset($_POST["checkedSpeaker"])){   
                        echo 3;
                    } else if($GLOBALS["bool"] == true){
                        echo 2;
                    } else echo 0;
    ...
    if (n == 1 && !validateForm()) {
        ...
        switch (currentTab) {
            case 0:
                ...
                break;
            case 1:
                error.innerHTML = "Seleziona un tutorial";
                error.style.visibility = "visible";
                break;
            case 2:
                error.innerHTML = "Seleziona uno speaker";
                error.style.visibility = "visible";
                break;
            default:
                break;
...
function validateForm() {
    ...
    switch (currentTab) {
        ...
        case 1:
            var radio = document.getElementsByName("checkedTutorial[]");
            for (var i = 0; i < radio.length; i++) {
                if (radio[i].checked == true) {
                    selected = true;
                }
            ...
            break;
        case 2:
            var radio = document.getElementsByName("checkedSpeaker[]");
            for (var i = 0; i < radio.length; i++) {
                if (radio[i].checked == true) {
                    selected = true;
                }
